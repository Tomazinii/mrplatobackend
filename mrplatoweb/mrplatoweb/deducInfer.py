...
def applyInferRule(rule,lines):
    ...
    nconcl = None
    # for d in dic:
    #     print(f'dic[{d}] : {dic[d]} - type: {type(dic[d])}')
    if r:
        r1, msg1, nconcl = eqv.substitue(dic, concl)
...
def apply(lineIndex,proveLines,rule,selectdLinesIndices):
    ...
    concl = applyInferRule(rule,selLines)
...
#
#     try:
#         concl = applyInferRule(rule,selectdLines)
#     except:
#         print("Essa regra não pode ser aplicada. Tente novamente")
#     return concl
...
def apply_exist_gener(new_var, q_vars, selected_term, index_terms, form, form_vars):
    ...
    if r: # If all restrictions are respected, r1eplace the term by the new variable in the formula
        form_c = copy.deepcopy(form)
