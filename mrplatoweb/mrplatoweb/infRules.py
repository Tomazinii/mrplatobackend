...
class InferRule(): 
    '''
        ...
        Arguments:
            ...
            l_premis: the liste of premisses.
...
def generateInfRules(l_rules):
    ...
    while ir < len(l_rules):
        ...
        name = l_rules[ir][1]
        rule = l_rules[ir][2]
        l_prem, concl = generateInfRuleRepresentarion(name,rule)
...
#-----------------------------------------------------------------------------
# inference rule = {name: (list_of_premissas, conclusion)}
def criateInfRules():
    '''
        Criates a table of inference rules.
