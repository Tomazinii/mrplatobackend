...
class ExerciseRepository(ExerciseRepositoryInterface):
    ...
    @classmethod
    def get_list(self, index_list_exercise: int = None) -> List[Exercises]:
        if index_list_exercise != None:
            file = ListExerciseModel.objects.filter(id=index_list_exercise)
            return file
        all_list = ListExerciseModel.objects.all()
        result = [Exercises.create(file=element.file.readlines(), list_name=element.list_name, id=element.id) for element in all_list]
