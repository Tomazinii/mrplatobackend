...
class GlobalConstants():
    ...
    equiv = u'\u21D4' # Bi-implecation operator ('⇔')
    ...
    list_of_functs = ['p', 'q', 'r', 's', 't', 'u']+[true,false]
...
class Fof(Form):
    ...
    def insertQuantifier(self, quants):
        self.quantifiers = quants
        return
...
def index_form2(pos, form):
    ...
    # print(f'opnd1: {opnd1}')
    # print(f'opnd2: {opnd2}')
    initial_pos = copy.copy(pos) # initial position of form
    len_opnd1 = len(str(opnd1))  # length of operand 1
    if type(opnd1) is Form2:  # (X oper Y) oper Z
        pos += 1  # The opening parenthesis takes one position
        f_opnd1 = index_form(pos, opnd1)
        pos += 1  # Counts for the closing parenthesis

        # A Form2 as operand of another Form2 needs parenthesis
    ...
    if type(opnd2) is Form2:  # X oper (Y oper Z)
        # print(f'pos of opnd1 in form2: {pos}')
        # The initial position of the second operand is
        # position of the first operant +
        # the length of the  first operand +  the length of oper + 2
        # (to count the '(' character), counted from
        # the initial position of the formula itself
...
def generate_represent(form):
    ...
    if l == 0:
    ...
    elif (l == 1) & (form[0] in GlobalConstants.list_of_all_conecs):  # Those operations must have an operand - [ '~','^','v','->','<->']
        err_message = 'Error: an operand is missing.'
        return False, err_message, None
    elif (l == 1) & (form[0] in [ '⊤','⊥']):  # Constants
        return True, '', Form0(form[0])
    elif (l == 1) & (form[0] in GlobalConstants.list_of_functs):  # Those operations must have an operand - ['p', 'q', 'r', 's', 't','u']
        return True, '', Form(form[0])
    elif (l == 1) & (form[0] in [ '⊤','⊥']):  # Constants
        return True, '', Form0(form[0])
    elif (l == 1):
        err_message= 'Error: wrong formula.'
        return False, err_message, None
    elif (l == 2) & (form[0] == GlobalConstants.c_not):
        r, err_message, opnd1 = generate_represent(form[1])
        nf = Form1(GlobalConstants.c_not,opnd1)
        return r, err_message, nf
    elif (l == 2) & (form[0] in GlobalConstants.list_of_cnecs ):  # Those operations must have two operands - [ '^','v','->','<->']
        err_message = 'Error: the first operand is missing.'
        return False, err_message, None
    elif (l == 2) & (form[1] in GlobalConstants.list_of_cnecs):  # Those operations must have two operands - [ '^','v','->','<->']
        err_message = 'Error: the second operand is missing.'
        return False, err_message, None
    elif (l == 2) & (form[0] in GlobalConstants.list_of_functs):  # Those operations must have an operand - ['p', 'q', 'r', 's', 't', 'u']
        ...
        return False, err_message, None
    elif (l > 2)  and form[0] in GlobalConstants.list_of_quants:  #['∀', '∃']

        r, err_message, oper, opnd1,opnd2 = handle_precedence_of_operators(form)
        print(f'rp1: {r} - opnd1: {opnd1} - opnd2:{opnd2}')
        quant_list = []
